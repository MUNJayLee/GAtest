...
best_direct_proportion=sum(bestz_ijdc)/C_max;
tempt_tans=TC(1,:)-C_IDT(4,:);
tempt_tans(tempt_tans<=0)=[];
best_direct_proportion=best_direct_proportion+0.08;
best_T_trans_avg=mean(tempt_tans);
bestRESC(1,:)=length(find(besty_ijdc(1,:)==0));
TEMPT=TC(1,:)-C_IDT(4,:);
...
best_train_cost_all=bestfit2_1+bestfit2_2;
best_train_cost_avg=best_train_cost_all/J_max;
